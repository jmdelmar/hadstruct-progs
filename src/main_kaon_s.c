...
#define NF 1
char part_str[7] = "kaon";
char flav_str[7] = "strange";
...
{   
  ...
  for(int isrc=0; isrc<rp.nsp; isrc++) {
	...
	mg_state.params.mu = rp.act.mu_s;
	DDalphaAMG_update_parameters(&mg_state.params, &mg_state.status);
	mg_state.current_mu_sign = plus;
	...
	if(am_io_proc)
	  printf("Done %ssequential inversion in %g sec\n", flav_str, qhg_stop_watch(t0));
	...
	thrp.corr = qhg_nn_thrp(sol_s, seq_sol, gf, sco, thrp_snk);
	...
	if(true) {
	  ...
	  asprintf(&fname, "%s/thrp_%s_%s_%s_%s_dt%02d_mom_%+d_%+d_%+d.%s.h5",
		   rp.corr_dir, part_str, srcstr, smrstr_s, apestr, 
		   thrp_snk.dt, mom_vec[0], mom_vec[1], mom_vec[2], flav_str);
	  char *group;
	  asprintf(&group, "thrp/%s/dt%02d/%s",
		   srcstr, thrp_snk.dt, flav_str);
	...
	if(false) {
	  t0 = qhg_stop_watch(0);
	  thrp.corr = qhg_nn_thrp_der2(sol_s, seq_sol, gf, sco, thrp_snk);
	  thrp.flav = strange;
	  thrp.dt = thrp_snk.dt;
	...
	for(int der_order = 2; der_order <=3; der_order++) {
	  t0 = qhg_stop_watch(0);
	  qhg_der_correlator corr = qhg_nn_thrp_der(sol_s, seq_sol, gf, sco, thrp_snk, der_order, to_skip, true, true);
	  ...
	  */
	  if(true) {
	    ...
	    asprintf(&fname, "%s/thrp_der%d_%s_%s_%s_%s_dt%02d_mom_%+d_%+d_%+d.%s.h5",
		     rp.corr_dir, der_order, part_str, srcstr, smrstr_s, apestr, 
		     thrp_snk.dt, mom_vec[0], mom_vec[1], mom_vec[2], flav_str);
	    char *group;
	    asprintf(&group, "thrp/%s/%s/dt%02d/%s",
		     srcstr, proj_to_str(thrp_snk.proj),
		     thrp_snk.dt, flav_str);
