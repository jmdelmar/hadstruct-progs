...
#define NF 3
char flav_str[NF][6] = {"pion","kaon_u","kaon_s"};
  ...
  for(int isrc=0; isrc<rp.nsp; isrc++) {
    ...
    */
    ...
    for(int flav=0; flav<NF-1; flav++) {
      mg_state.init.mu = flav == 0 ? rp.act.mu_l : rp.act.mu_s;
      for(int i=0; i<NS*NC; i++) {
	mg_invert(sol_f[flav][i], src[i], 1e-9, plus, &mg_state);
      }
    }
    if(am_io_proc)
    ...
    */
	...
	qhg_gauss_smear(sol_sm_s[CS(sp,co)], sol_s[CS(sp,co)], gf_ape, alpha_gauss, n_gauss);
      }
    if(am_io_proc)
      printf("Done smearing in %g sec\n", qhg_stop_watch(t0));  
    ...
    */
    t0 = qhg_stop_watch(0);
    qhg_correlator mesons_l = qhg_mesons(sol_sm_u, sol_sm_u, sco);
    qhg_correlator_shift(mesons_l, mesons_l.origin);
    qhg_correlator mesons_s = qhg_mesons(sol_sm_u, sol_sm_s, sco);
    qhg_correlator_shift(mesons_s, mesons_s.origin);
    if(am_io_proc)
    ...
    */
    if(true) {
      t0 = qhg_stop_watch(0);      
      char *fname;
      asprintf(&fname, "%s/twop_pion_%s_%s_%s.h5", rp.corr_dir, srcstr, smrstr, apestr);
      char *group;
      asprintf(&group, "twop_pion/%s/", srcstr);      
      qhg_write_mesons(fname, mesons_l, group);
      if(am_io_proc)
	printf("Wrote %s in %g sec\n", fname, qhg_stop_watch(t0)); 

      asprintf(&fname, "%s/twop_kaon_%s_%s_%s.h5", rp.corr_dir, srcstr, smrstr, apestr);
      asprintf(&group, "twop_kaon/%s/", srcstr);      
      qhg_write_mesons(fname, mesons_s, group);
      if(am_io_proc)
	printf("Wrote %s in %g sec\n", fname, qhg_stop_watch(t0)); 
      free(fname);
      free(group);
    }
    qhg_correlator_finalize(mesons_l);
    qhg_correlator_finalize(mesons_s);
	...
	*/
    	mg_state.init.mu = flav == 2 ? rp.act.mu_s : rp.act.mu_l;
    	
	t0 = qhg_stop_watch(0);
	for(int i=0; i<NS*NC; i++) {
	  mg_invert(seq_sol[i], seq_src[i], 1e-9, minus, &mg_state);
	}
    	...
    	qhg_spinor_field *fwd[2] = {sol_u, sol_s};
    	...
    	if(true) {
    	  t0 = qhg_stop_watch(0);
    	  char *fname;
    	  asprintf(&fname, "%s/thrp_%s_%s_%s_dt%02d.%s.h5",
    		   rp.corr_dir, srcstr, smrstr, apestr, 
