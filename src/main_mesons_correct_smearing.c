...
{   
  ...
  for(int isrc=0; isrc<rp.nsp; isrc++) {
    ...
    int n_gauss_f[] = {n_gauss_l, n_gauss_l, n_gauss_s};
    double alpha_gauss_f[] = {alpha_gauss_l, alpha_gauss_l, alpha_gauss_s};
	...
	for(int flav=0; flav<NF-1; flav++) {
	  ...
	  qhg_gauss_smear(src_f[flav][CS(sp,co)], aux, gf_ape, alpha_gauss_f[flav+1], n_gauss_f[flav+1]);
	}
    ...
    for(int flav=0; flav<NF-1; flav++) {
	 ...
	 //mg_invert(sol_f[flav][i], src_f[flav][i], 1e-9, plus, &mg_state);
      }
    }
    if(am_io_proc)
